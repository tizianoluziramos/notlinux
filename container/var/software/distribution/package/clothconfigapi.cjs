...
const defaultFormat = 'YYYY-MM-DD HH:mm:ss'; // Formato de fecha y hora por defecto
const dayFormat = 'DD'; // Formato de día
const weekFormat = 'WW'; // Formato de semana
const monthFormat = 'MMMM YYYY'; // Formato de mes
const timeFormat = 'HH:mm:ss'; // Formato de hora
const dateFormat = 'YYYY-MM-DD'; // Formato de fecha
const isoFormat = 'ISO 8601'; // Formato ISO 8601
...
function getCurrentMonth(timezone = defaultTimezone) {
  return new Date().toLocaleString('en-US', { month: 'long', timeZone: timezone });
}
...
function getStartOfMonth(date = new Date()) {
  return new Date(date.getFullYear(), date.getMonth(), 1).toLocaleString('en-US');
}

function getEndOfMonth(date = new Date()) {
  return new Date(date.getFullYear(), date.getMonth() + 1, 0).toLocaleString('en-US');
}
